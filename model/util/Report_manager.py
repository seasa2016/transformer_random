import time
from datetime import datetime
from .statistics import Statistics
from .Logger import logger

def build_report_manager(opt,checkpoint=None):
    start_time = -1

    if(opt.tensorboard):
        from tensorboardX import SummaryWriter

        tensorboard_log_dir = opt.save_model

        progress_step = 0
        if(opt.train_from):
            tensorboard_log_dir = checkpoint['reporter']['loc']
            progress_step = checkpoint['reporter']['progress_step']
            start_time = checkpoint['reporter']['start_time']


        writer = SummaryWriter( tensorboard_log_dir, comment='Unmt' )
    else:
        writer = None
    
    report_mgr = ReportMgr(opt.report_every,loc=tensorboard_log_dir,step = progress_step,start_time=start_time,tensorboard_writer=writer)

    return report_mgr

class ReportMgrBase(object):
    """
    Report Manager Base class
    Inherited classes should override:
        * `_report_training`
        * `_report_step`
    """
    def __init__(self,report_every,loc,start_time=-1,step=0):
        """
        Args:
            report_every(int): Report status every this many sentences
            start_time(float): manually set report start time. Negative values
                means that you will need to set it later or use `start()`
        """
        self.report_every = report_every
        self.progress_step = step
        self.start_time = start_time
        self.loc = loc
    
    def start(self):
        self.start_time = time.time()
    
    def log(self,*args,**kwargs):
        logger.info(*args,**kwargs)
    
    def report_training(self,step,num_steps,learning_rate,
                        report_stats,multigpu=False):
        """
        This is the user-defined batch-level traing progress
        report function.

        Args:
            step(int): current step count.
            num_steps(int): total number of batches.
            learning_rate(float): current learning rate.
            report_stats(Statistics): old Statistics instance.
        Returns:
            report_stats(Statistics): updated Statistics instance.
        """
        if(self.start_time < 0):
            raise ValueError("havent start the reportmgr yet")

        if(multigpu):
            report_stats = Statistics.all_gather_stats(report_stats)
        
        if( step % self.report_every == 0):
            self._report_training(
                step,num_steps,learning_rate,report_stats)
            self.progress_step += 1
        
        return Statistics()

    def _report_training(self,*args,**kwargs):
        raise NotImplementedError()

    def report_step(self,lr,step,train_stats=None,valid_stats=None):
        """
        Report stats of a step

        Args:
            train_stats(Statistics): training stats
            valid_stats(Statistics): validation stats
            lr(float): current learning rate
        """
        self._report_step(
            lr, step, train_stats=train_stats, valid_stats=valid_stats)

    def _report_step(self, *args, **kwargs):
        raise NotImplementedError()

class ReportMgr(ReportMgrBase):
    def __init__(self,report_every,loc,step=0,start_time=-1,tensorboard_writer=None):
        """
            A report manager that writes statistics on standard output as well as
            (optionally) TensorBoard

            Args:
                report_every(int): Report status every this many sentences
                tensorboard_writer(:obj:`tensorboard.SummaryWriter`):
                    The TensorBoard Summary writer to use or None
        """
        super(ReportMgr,self).__init__(report_every,loc,start_time,step)
        self.tensorboard_writer = tensorboard_writer

    def maybe_log_tensorboard(self,stats,prefix,learning_rate,step):
        if(self.tensorboard_writer is not None):
            stats.log_tensorboard(prefix,self.tensorboard_writer,learning_rate,step)

    def _report_training(self,step,num_steps,learning_rate,report_stats):
        """
        see base class method 'ReportMgrBase.report_training'
        """
        report_stats.output(step,num_steps,learning_rate,self.start_time)

        self.maybe_log_tensorboard(report_stats,
                                    "progress",
                                    learning_rate,
                                    self.progress_step)
        report_stats = Statistics()

    def _report_step(self,lr,step,train_stats=None,valid_stats=None):
        """
        See base class method `ReportMgrBase.report_step`.
        """
        if train_stats is not None:
            self.log('Train perplexity: %g' % train_stats.ppl())
            self.log('Train accuracy: %g' % train_stats.accuracy())

            self.maybe_log_tensorboard(train_stats,
                                       "train",
                                       lr,
                                       step)

        if valid_stats is not None:
            self.log('Validation perplexity: %g' % valid_stats.ppl())
            self.log('Validation accuracy: %g' % valid_stats.accuracy())

            self.maybe_log_tensorboard(valid_stats,
                                       "valid",
                                       lr,
                                       step)
    
    def data(self):
        """
            this is set for easily saving the parameter
            now we save:
                        self.progress_step
                        self.start_time
                        self.loc
        """
        return  {
                    "progress_step":self.progress_step,
                    "start_time":self.start_time,
                    "loc":self.loc
                }



