...
petitle = data.iloc[:,3].tolist()
psongs = data.iloc[:,4].tolist()
ptags = data.iloc[:,5].tolist()
...
with open('raw_data.csv', 'w') as f:
  with open('no_tags.csv', 'w') as fn:
    ...
    for pi, pct, pet, ps, pt, pl in zip(pid, pctitle, petitle, psongs, ptags, plang):
      if pi in le_mapping:
      ...
      if str(pi) not in bad_playlist:
        ...
        if '21071' not in pt and '20408' not in pt and '20228' not in pt and '20441' not in pt:
          ...
          pt_y = ['y_' + str(y) for y in pt if y in tag_year]
          pt_o = ['o_' + str(o) for o in pt if o in tag_other]
          pt_l = ['l_' + str(l) for l in pt if l in tag_lang]
          pt_a = ['a_' + str(a) for a in pt if a in tag_artist]
